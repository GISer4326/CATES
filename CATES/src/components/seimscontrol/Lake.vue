<template>
  <div>
    <transition name="fade">
      <Modal v-model="lakeShowTemp" draggable scrollable footer-hide :mask="false" title="Extraction of lake-watershed attributes" width="650px"
        ...
        <div v-show="activeKey == '1'" style="margin: 20px;margin-top: 20px;">
          ...
          <div v-show="activeKey == '2'" style="margin: 20px;margin-top: 10px;margin-bottom: 0px">
            ...
            <Table border ref="selection1" :columns="tablecolumns1" :data="tabledata1" height="136">
          ...
          <div  v-show="activeKey == '3'" style="margin: 20px;margin-bottom: 0px;">
            ...
            <div style="margin-bottom: 5px;">
              ...
              <!-- http://210.26.118.100:8083/api/infboy/uploadDEM -->
              <Space>
          ...
          <!-- <TabPane label="统计数据导出">
            <div>
              ...
              <div id="landuseChart1" style="height: 300px;width: 500px;margin-top: 30px;"></div>
    ...
    <transition name="fade">
      <Modal v-model="staticDataModal" draggable scrollable footer-hide :mask="false" title="Report" z-index="999" width="1300px">
        <Table :columns="columns" :data="data" border max-height="500" size="default" no-data-text="Nodata" ref="table2" :loading="tableloading2" style="margin: 20px;"></Table>
        <Space class="ivu-mt" wrap>
          <Button type="primary" @click="exportData(1)" style="margin-left: 20px;"><Icon type="ios-download-outline"></Icon> Export source data</Button>
        </Space>
      </Modal>
    </transition>
  </div>
...
export default {
  ...
  watch: {
    ...
    lakeShow(newVal, oldVal) {
      this.lakeShowTemp = newVal
    },
    immediate: true,
  ...
  data() {
    ...
    return {
      ...
      Population_Count:[
        {
          title: 'Population_Count_max(people)',
  ...
  methods: {
    ...
    namedProject(name) {
      ...
      this.$refs[name].validate((valid) => {
        if (valid) {
          ...
          console.log(this.projectName)
          this.$store.commit("SET_PROJECT_NAME", this.projectName);
          this.$Notice.success({
            title: '项目命名完成',
            duration: 3
          });
    ...
    updatesuccess1(index){
      ...
      //DEM,执行burn-in
      if(this.tabledata1[index].Factor == "dem"){
        let params = new URLSearchParams();
        params.append("projectName", this.projectName);
        params.append("username", this.username);
    ...
    selectChange1(value, index){
      console.log(value)
      this.sourceSelectdata[this.tabledata1[index].Factor] = value
    ...
    static(){
      ...
      var _this = this;
      //生成流域范围，并渲染.POI或者数据源改变则重新请求后端提取流域范围
      if (JSON.stringify(this.oldPOIdata) !== JSON.stringify(this.POIdata) || JSON.stringify(this.sourceSelectdata) !== JSON.stringify(this.old_sourceSelectdata)){
        _this.$Notice.info({
            name: 'basin',
        ...
        params1.append("DEMsourcetype", DEMsourcetype);
        params1.append("radius", "");
        params1.append("radiusType", "");
        ...
        //       console.log("渲染")
        //       for (var json of res.data) {
        //         _this.addGeoJSONToMap('basinLayer', JSON.stringify(json), "red");
        //       }
        //       _this.$Spin.hide();
        //       this.staticDataModal = true
        //     }
      ...
      // // 面积
      // if (this.oldPOIdata && JSON.stringify(this.oldPOIdata) === JSON.stringify(this.POIdata) && this.textDatas["text"].length != 0 && JSON.stringify(this.sourceSelectdata) === JSON.stringify(this.old_sourceSelectdata)){
      //       this.textData["text"] = this.textDatas["text"][this.pointSelect - 1]
      //       console.log(this.textData["text"])
      //       var myDiv = document.getElementById('text');  
      //       myDiv.innerHTML = "Basin area: " + JSON.stringify(this.textData["text"].area) + "㎡<br>Glacier area:" + JSON.stringify(this.textData["text"].glacier) + "㎡"
      // }
      // else{
      //     console.log("请求后端")
      //     let params = new URLSearchParams();
      ...
      //     request
      //         .post('/file/statisticsArea', params)
      //         .then(res => {
      //             if (res != null) {
      //                 console.log("统计")
      //                 console.log(res.data)
      //                 this.textDatas["text"] = res.data
      //                 this.textData["text"] = res.data[this.pointSelect - 1]
      ...
      var timer1 = window.setInterval(() => {
        setTimeout( () =>{
          ...
          polling.append("username", sessionStorage.getItem('username'));
          polling.append('filepathandname',"data_prepare/spatial/basin.tif") //生成的最后一个文件是最后一个点的流域范围tif，以此作为子流域划分结束的标志
          request.post("/Polling", polling)
            .then(res=>{
              console.log(res.data.errCode);
              if (res.data.errCode == 200) {
      ...
      const pollData = setInterval(() => {
        ...
        if (this.basinRespons) {
          ...
          for (let type of this.TypeList) {
            ...
            // 数值型
            if (type == "Human Footprint 2009" || type == "Total GDP 2015" || type == "Per Capita GDP 2015" || type == "HDI 2009" || type == "Nighttime Lights 2013" || type == "Road Density" || type == "Elevation" || type == "Slope" || type == "Area" || type == "NDVI" ||  type == "Porosity" ||  type == "LAI" ||  type == "EVI" ||  type == "ET" ||  type == "NPP" ||  type == "GPP" ||  type == "Subsurface Permeability" || type == "Population Count"|| type == "Population Density"|| type == "Soil Properties") {
                if (this.oldPOIdata && JSON.stringify(this.oldPOIdata) === JSON.stringify(this.POIdata) && this.textDatas[type].length != 0 && JSON.stringify(this.sourceSelectdata) === JSON.stringify(this.old_sourceSelectdata)){
            ...
            //类型
                ...
                console.log("类型")
                if (this.oldPOIdata && JSON.stringify(this.oldPOIdata) === JSON.stringify(this.POIdata) && this.chartDatas[type].length != 0 && JSON.stringify(this.sourceSelectdata) === JSON.stringify(this.old_sourceSelectdata) ){
                    this.disabled = false;
    ...
    //1:基础数据；2：环境因子
    checkSourceChangeByFactorType(Factor){
      return this.sourceSelectdata[Factor] == this.old_sourceSelectdata[Factor]
    },
    ...
    pointSelectChange() {
      ...
      for (let type of this.TypeList) {
        ...
        switch (type) {  
          case "NDVI":
            console.log("NDVI")
            chartInstance = echarts.getInstanceByDom(document.getElementById('NDVIChart1'));
            break;  
          case "Landuse":
            console.log("Landuse")
            chartInstance = echarts.getInstanceByDom(document.getElementById('landuseChart1'));
            break;   
          case "Soil":
            console.log("Soil")
            chartInstance = echarts.getInstanceByDom(document.getElementById('soilChart1'));
            break;  
          case "Geological":
            console.log("Geological")
            chartInstance = echarts.getInstanceByDom(document.getElementById('geologicalChart1'));
            break;  
          case "Porosity":
            console.log("Porosity")
            chartInstance = echarts.getInstanceByDom(document.getElementById('porosityChart1'));
            break;    
        }
        chartInstance.showLoading();
        console.log(this.pointSelect)
        if (type == "NDVI" ||  type == "Porosity") {
            // 简单插入文本
            // var myDiv = document.getElementById('landuseChart1');  
            ...
            this.chartData[type] = this.chartDatas[type][this.pointSelect - 1]
            // var chartDom = document.getElementById('landuseChart1');
    ...
    initChart() {
        // $('#landuseChart1').css('height', '200px')
        console.log("初始化图表")
        var chartDom = document.getElementById('NDVIChart1');
        var myChart = echarts.init(chartDom);
        var chartDom = document.getElementById('landuseChart1');
        myChart = echarts.init(chartDom);
        var chartDom = document.getElementById('soilChart1');
        myChart = echarts.init(chartDom);
        var chartDom = document.getElementById('porosityChart1');
        myChart = echarts.init(chartDom);
        var chartDom = document.getElementById('geologicalChart1');
        myChart = echarts.init(chartDom);
